...
export class UserService {
    ...
    async createUser(dto: CreateUserDto, createdBy?: string): Promise<User> {
        ...
        const existingUserByPhone = await this.userRepository.existsByPhoneNumber(validatedDto.phoneNumber);
        ...
        if (validatedDto.document) {
            const existingUserByDocument = await this.userRepository.existsByDocument(validatedDto.document);
