...
export class UserRepository extends BaseRepository<User, string> implements IUserRepository {
    ...
    async findAll(): Promise<User[]> {
        try {
            ...
            throw new DatabaseError(`Failed to find all users: ${errorMessage}`, error instanceof Error ? error : undefined);
        }
    }
    ...
    async findBy(criteria: Record<string, any>): Promise<User[]> {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            throw new DatabaseError(`Failed to find users by criteria: ${errorMessage}`, error instanceof Error ? error : undefined);
        }
    }
    ...
    async findByApartment(unitCode: string): Promise<User[]> {
        try {
            ...
            const phoneNumbers = result.Items
                .map(item => item.sk.replace('USER#', '').split('#')[0])
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            throw new DatabaseError(`Failed to find users by apartment: ${errorMessage}`, error instanceof Error ? error : undefined);
        }
    }
    ...
    async existsByDocument(document: string): Promise<boolean> {
        try {
            ...
            const command = new ScanCommand({
                TableName: this.tableName,
                FilterExpression: 'begins_with(pk, :userPrefix) AND sk = :sk AND (#document = :document OR cpf = :document)',
                ExpressionAttributeValues: {
                    ':userPrefix': 'USER#',
                    ':sk': 'PROFILE',
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            throw new DatabaseError(`Failed to check document existence: ${errorMessage}`, error instanceof Error ? error : undefined);
        }
    }
    ...
    async save(user: User): Promise<User> {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            throw new DatabaseError(`Failed to save user: ${errorMessage}`, error instanceof Error ? error : undefined);
        }
    }
