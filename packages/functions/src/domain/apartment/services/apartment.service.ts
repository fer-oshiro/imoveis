...
export default class ApartmentService {
  ...
  async getApartmentDetails(unitCode: string): Promise<ApartmentDetails> {
    try {
      ...
      if (!apartment) {
        throw new DomainError(`Apartment with unit code ${unitCode} not found`, 'APARTMENT_NOT_FOUND', 404);
      }
  ...
  async createApartment(dto: CreateApartmentDto, createdBy?: string): Promise<Apartment> {
    try {
      ...
      if (existingApartment) {
        throw new DomainError(`Apartment with unit code ${dto.unitCode} already exists`, 'APARTMENT_ALREADY_EXISTS', 409);
      }
  ...
  async updateApartment(unitCode: string, dto: UpdateApartmentDto, updatedBy?: string): Promise<Apartment> {
    try {
      const apartment = await this.apartmentRepository.findByUnitCode(unitCode);
      if (!apartment) {
        throw new DomainError(`Apartment with unit code ${unitCode} not found`, 'APARTMENT_NOT_FOUND', 404);
      ...
      if (dto.baseRent !== undefined || dto.cleaningFee !== undefined) {
        apartment.updatePricing(
          dto.baseRent ?? apartment.baseRentValue,
          dto.cleaningFee,
          updatedBy
        );
      }
